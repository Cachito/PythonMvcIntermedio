...
class Ui_main_view(object):
    def setupUi(self, main_view):
        ...
        self.btn_base.clicked.connect(self.create_data)
        # botón tablas
        ...
        # tabla
        self.tb_noticias = QtWidgets.QTableView(self.centralwidget)
        self.tb_noticias.setGeometry(QtCore.QRect(10, 420, 481, 192))
        self.tb_noticias.setObjectName("tb_noticias")
    ...
    def refresh(self):
        ...
        """
        data = self.controller.get_noticias()
        columns = 6
        rows = len(data)
        self.notas_model = QStandardItemModel(rows, columns)
        #self.notas_model.setHorizontalHeaderLabels(['Id', 'Fecha', 'Medio', 'Sección', 'Título', 'Cuerpo'])
        ...
        for r in data:
            ...
            while c < 6:
                ...
                #print(value, item)
                self.notas_model.setItem(r, c, QTableWidgetItem(item))
                c += 1

        self.tb_noticias.setModel(self.notas_model)

    def clean_noticias(self):
        """
        vacía tableview
        """
